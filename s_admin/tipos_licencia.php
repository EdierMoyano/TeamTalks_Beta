...
if (isset($_GET['eliminar']) && !empty($_GET['eliminar'])) {
    ...
    try {
        ...
        if ($tiene_licencias) {
            ...
            $stmt = $conn->prepare("DELETE FROM tipo_licencia WHERE id_tipo_licencia = ?");
            $stmt->execute([$id_eliminar]);
...
try {
    ...
    $stmt = $conn->prepare("SELECT tl.*, 
                           (SELECT COUNT(*) FROM licencias WHERE id_tipo_licencia = tl.id_tipo_licencia) as total_licencias
                           FROM tipo_licencia tl
                           $where
                           ORDER BY tl.id_tipo_licencia DESC
                        ...
                        <form method="GET" class="row g-3">
                            <div class="col-md-6">
                                <div class="input-group">
                                    <input type="text" class="form-control" name="buscar" placeholder="Buscar por nombre..." value="<?php echo htmlspecialchars($busqueda); ?>">
                            ...
                            <table class="table table-striped table-hover">
                                ...
                                <?php if (count($tipos_licencia) > 0): ?>
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo htmlspecialchars($tipo['licencia']); ?></td>
                                        <td><?php echo $tipo['duracion']; ?></td>
                                        <td><span class="badge bg-primary"><?php echo $tipo['total_licencias']; ?></span></td>
                                        <td>
                                            <a href="tipos_licencia_editar.php?id=<?php echo $tipo['id_tipo_licencia']; ?>" class="btn btn-sm btn-warning"><i class="bi bi-pencil"></i></a>
                                            <?php if ($tipo['total_licencias'] == 0): ?>
                                                <a href="#" class="btn btn-sm btn-danger" onclick="confirmarEliminar(<?php echo $tipo['id_tipo_licencia']; ?>, '<?php echo htmlspecialchars($tipo['licencia']); ?>')"><i class="bi bi-trash"></i></a>
                                            <?php else: ?>
                                    ...
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <tr><td colspan="5" class="text-center">No se encontraron tipos de licencia.</td></tr>
                                <?php endif; ?>
                                </tbody>
                            </table>
                        </div>
    ...
    <div class="modal fade" id="eliminarModal" tabindex="-1" aria-labelledby="eliminarModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">¿Está seguro de que desea eliminar el tipo de licencia <span id="nombreTipo"></span>?</div>
